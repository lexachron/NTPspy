...
class DiskStorageProvider(StorageProvider):
    ...
    def finalize_session(self, session_id: int, overwrite: bool = False) -> str:
        with self.lock:
            ...
            text_file_path = os.path.join(self.base_path, self.sessions[session_id][BufferType.TEXT.value])
            handle = None
            ...
            if not handle:
                # autogenerated filename if none provided
                timestamp = datetime.datetime.now().strftime("%y%m%d-%H%M%S")
                handle = f"{timestamp}-{session_id:08x}"

            buffer_path = os.path.join(self.base_path, self.sessions[session_id][BufferType.DATA.value])
            final_path = os.path.join(self.base_path, handle)
